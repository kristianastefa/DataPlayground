...
class UserSimulator():
    ...
    def run_simulation(self):
        ...
        while self.running:
            ...
            response = requests.get(self.recommendation_endpoint, params={'userid':int(userid), 'itemid': int(itemid)})
            estimated_rating = response.json()['estimated_rating']
    
            # send the true rating and estimated rating to the feedback endpoint
            response = requests.get(self.feedback_endpoint, params={'userid':int(userid), 'itemid': int(itemid), 'rating':rating, 'estimated_rating':estimated_rating})
            rmse = response.json()['rmse']
            unique_us = response.json()['unique_users']
            print(rmse ,unique_us, flush = True)
        self.daemon = None
